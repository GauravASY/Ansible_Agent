...
interface DatabaseNodeData {
  ...
  latency?: number;
  details?: string; // e.g., "PostgreSQL 14, 500GB"
}

export function DatabaseNode({ data, nodeId, selected, position }: {
  data: DatabaseNodeData;
  nodeId: string;
  selected: boolean;
  position: { x: number; y: number };
}) {
  const statusColors: Record<DatabaseNodeData["status"], string> = {
    online: "bg-green-500",
